...
/**
  ...
  */
uint8_t OLED_DisplayBuffer[8][128];
...
{
	I2C1_WriteByte(OLED_ADDR, OLED_DATA, byte);
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteBuffer(uint8_t *pbuffer, uint8_t Count)
{
	I2C1_WriteBuffer(OLED_ADDR, OLED_DATA, pbuffer, Count);
}
...
{
	I2C1_WriteByte(OLED_ADDR, OLED_CMD, cmd);
}
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteBuffer(OLED_DisplayBuffer[j], 128);
	}
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteBuffer(&OLED_DisplayBuffer[j][X], Width);
		}
	}
}
...
void OLED_Reverse(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			OLED_DisplayBuffer[j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void OLED_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			if (i >= 0 && i <= 127 && j >=0 && j <= 63)			//超出屏幕的内容不显示
			{
				OLED_DisplayBuffer[j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
			}
		}
	}
}
...
void OLED_DrawPoint(int16_t X, int16_t Y)
{
	...
	{
		/*将显存数组指定位置的一个Bit数据置1*/
		OLED_DisplayBuffer[Y / 8][X] |= 0x01 << (Y % 8);
	}
}
...
uint8_t OLED_GetPoint(int16_t X, int16_t Y)
{
	...
	{
		/*判断指定位置的数据*/
		if (OLED_DisplayBuffer[Y / 8][X] & 0x01 << (Y % 8))
		{
			return 1;	//为1，返回1
		}
	}
